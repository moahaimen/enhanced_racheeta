...
| Profile edits | `update_seeker_profile` | A PATCH applies only the submitted fields to the locked profile row and writes only those columns, so a concurrent privacy opt-out is never reversed by an unrelated edit; child rows (experiences, education, skills, languages, credentials) are locked, refreshed and re-validated (cross-field rules such as the experience date range) before saving; saving a candidate decides on the locked candidate row (order employer → candidate). |
