...
| Profile edits | `update_seeker_profile` | A PATCH applies only the submitted fields to the locked profile row and writes only those columns, so a concurrent privacy opt-out is never reversed by an unrelated edit; saving a candidate decides on the locked candidate row (order employer → candidate). |
