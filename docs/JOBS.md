...
| Employer organisation | `Employer` created by an account (owner), optionally linked to a Phase 2 facility `ProviderProfile` | verified by Super Admin, then posts jobs, reviews applicants, searches talent within its plan. Verifying an organisation re-checks each of its PUBLISHED jobs against the agency invariant under the current identity (lock order employer → job) and suspends, with a transition and audit record, any job that no longer holds, so re-verification never makes an invalid job public again. Once verification is requested (PENDING) or granted (VERIFIED), the identity fields the administrator reviewed — `name`, `organization_type`, `governorate`, `provider_profile`, `is_recruitment_agency` — are locked for owners (`identity_locked`, 400); description, city and discoverability stay editable. Changing identity needs an administrator. |
| Employer members | `EmployerMembership(role OWNER|RECRUITER|VIEWER, status ACTIVE|ENDED)` | one live membership per account (DB constraint; adding locks the employer row, then the target account row, so two organisations adding the same account get one success and one typed `already_member`); seats limited by `recruiter.seats` |
