...
| `CanRecruit` | OWNER or RECRUITER (job writes, applicant transitions, interviews, invitations, messages); VIEWER is read-only. The employer workspace mirrors this: New Job and talent search are shown only to OWNER/RECRUITER, the job editor renders read-only for a VIEWER who reaches it by URL, and the applicants page shows a VIEWER the applicant data but no transition, interview, reason or messaging control (employer-side messaging is part of applicant review, so a VIEWER is not a party to the thread). An unknown or missing role gets no write control. UX only, the backend stays authoritative. |
