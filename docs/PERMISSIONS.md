...
| `CanRecruit` | OWNER or RECRUITER (job writes, applicant transitions, interviews, invitations, messages); VIEWER is read-only. The employer workspace mirrors this: New Job and talent search are shown only to OWNER/RECRUITER, and the job editor renders read-only for a VIEWER who reaches it by URL — UX only, the backend stays authoritative. |
