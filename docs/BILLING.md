...
5. Admin can `reject` (PENDING), `suspend`/`cancel` (ACTIVE) and re-activate a SUSPENDED one; every step is audited. Every lifecycle transition locks the billing account row first (then the subscription row), and every paid write resolves its entitlement through `entitlements_for(..., lock=True)` on that same locked row, so a revocation that committed before the write is seen and one that comes later waits — entitlement, quota, consumption and the written row are one decision on the committed state. The same holds on the seeker side: applying locks the seeker's billing account, so a revoked paid plan resolves to the default plan and its current quota at commit time (consumption under an unlimited entitlement records the event but does not count toward a limited plan's bucket). An ACTIVE row whose term has ended is EXPIRED first, never suspended or cancelled: the staff subscription list normalises every elapsed ACTIVE row through `expire_subscription` (one locked, evented transition per row, bounded by the rows not yet normalised, idempotent) before filtering and serialising, and the direct suspend/cancel service paths and activation of another row do the same before they decide, so `?status=ACTIVE` never lists an elapsed term and the console never offers actions on one. Activation runs under a row lock on the billing account and re-reads the subscription status. Policy: **the administrator's activation supersedes any other live row** — another ACTIVE subscription, any SUSPENDED one and any newer PENDING request are cancelled (audited as `billing.subscription.cancelled`, reason "superseded by activation of …") before the row becomes ACTIVE, so the one-live-subscription constraint can never surface as a 500. Every status transition re-reads the row under a lock.
