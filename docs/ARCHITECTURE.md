...
│   │   └── accounts/   Account model, roles, JWT auth, /me
...
│       ├── components/ ApiActionButton, AsyncPage, LoadingOverlay, Spinner
...
| Auth | JWT (SimpleJWT): short-lived access token, rotating refresh token with blacklist. See `AUTHENTICATION.md`. |
...
| `accounts` | Account model, roles, register/login/refresh/logout, `/me` — done. Firebase exchange **(planned)**. |
...
- No router yet. Add `react-router` when the first real page arrives (Phase 1 web work).
...
gateways, Firebase. Each has a defined insertion point (see `DECISIONS.md`)
